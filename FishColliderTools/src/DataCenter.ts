...
module editor {
    ...
    export class DataCenter extends egret.EventDispatcher {
        ...
        public fishRegionMap: { [sign: number]: Array<{ posX: number, posY: number, radius: number }> } = {};

        public addFishRegion(sign:number,region:{ posX: number, posY: number, radius: number }){
        ...
        public removeFishRegion(sign:number,region:{ posX: number, posY: number, radius: number }){
            let regions = this.fishRegionMap[sign];
            if(regions == null){
                return ;
            }
            for(let i:number = 0;i<regions.length;i++){
                let item = regions[i];
                if(item.posX == region.posX && item.posY == region.posY && item.radius == region.radius){
                    regions.splice(i,1);
                    break;
                }
            }
        ...
        //保存远程路径
        public static SAVE_REMOTE_DATA_URL: string = "http://123.207.4.125/fishregion/saveresult.php";

        public static SAVE_JSON_DATA_URL: string = "http://123.207.4.125/fishregion/savejsonresult.php";
        //读取远程路径
        public static LOAD_REMOTE_DATA_URL: string = "http://123.207.4.125/fishregion/loadresult.php";

        public static Down_REMOTE_DATA_URL: string = "http://123.207.4.125/fishregion/result.csv";
        ...
        public SaveData() {
            ...
            req.method = egret.URLRequestMethod.POST;
            req.data = JSON.stringify(this.fishRegionMap);
            urlloader.load(req);
            urlloader.addEventListener(egret.Event.COMPLETE, onPostComplete, null);
        }
        ...
        protected formatResult(): string {
            ...
            for (let i in this.fishRegionMap) {
                let regions = this.fishRegionMap[i];
                regions.removeAll(v=>v.radius< 5);
                let region = "\"[";
                for (let j:number =0;j< regions.length;j++) {
                    let item = regions[j];
                    region += ("[" + item.posX + "," + item.posY + "," + Math.floor(item.radius) + "," + "]");
        ...
        protected parsePathDataFromCsvData(data: string) {
            if (data == null)
                return;
            this.fishRegionMap = JSON.parse(data);

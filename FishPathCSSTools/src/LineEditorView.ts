...
module editor{
    ...
    enum ControlHoverType{
        ...
        ControlHoverType_Self = 1,                  //悬停点自己，这是需要移动点本身;
        ControlHoverType_LastControlDot = 2,        //悬停前控制点， 需要移动控制前句柄;
        ControlHoverType_NextControlDot = 3         //悬停后控制点， 需要移动控制后句柄;
    }
    ...
    class ControlPointView extends eui.Group{
        ...
        //导出使用的线的间隔路径
        private static EXPORT_LINE_INTERVAL_DISTANCE:number = 10;
        ...
        //构造;
        constructor(x:number,y:number,type?, cx:number=0, cy:number=0){
            super();
            //记录坐标
            this.x = x;
            this.y = y;
            //记录控制点;
            this.mControlPointOffset = new egret.Point(cx, cy);
        ...
        protected getPosStr(dstPos):string{
            return "("+dstPos.x.toFixed(2)+","+dstPos.y.toFixed(2)+")"
        }
        ...
        protected drawControlLines(){
            ...
            if(this.mControlNextLine != null){
                this.removeChild(this.mControlNextLine);
                this.mControlNextLine = null;
            }       
            ...
            this.mControlNextLine.x = -this.x;
            this.mControlNextLine.y = -this.y;
            ...
            this.mControlLastDot.x =  -this.mControlPointOffset.x;
            this.mControlLastDot.y =  -this.mControlPointOffset.y;
            ...
            this.mControlNextDot.x = this.mControlPointOffset.x;
            this.mControlNextDot.y = this.mControlPointOffset.y;
        ...
        public onTouchBegin(touchEvent:egret.TouchEvent){
            ...
            this.hoverType = this.getHoverType(newPos.x, newPos.y);
        }
        ...
        public onTouchMove(touchEvent:egret.TouchEvent){
            ...
            let newPos = this.parent.globalToLocal(touchEvent.stageX, touchEvent.stageY)
            //点中自己;
            if(this.hoverType == ControlHoverType.ControlHoverType_Self){
                this.x = newPos.x;
                this.y = newPos.y;
                this.updateControlPoint();
            ...
            //点中前控制点
            else if(this.hoverType == ControlHoverType.ControlHoverType_LastControlDot){
                this.mControlPointOffset = new egret.Point( this.x - newPos.x,  this.y - newPos.y);
                //egret.log("hit lastControl, this.mControlPointOffset:"+this.getPosStr(this.mControlPointOffset)+" thisPos:"+this.getPosStr(this)+" newPos:"+this.getPosStr(newPos));
                this.updateControlPoint();
                this.refreshConnectLines();
            }
            //点中后控制点;
            else if(this.hoverType == ControlHoverType.ControlHoverType_NextControlDot){
                this.mControlPointOffset = new egret.Point(newPos.x - this.x, newPos.y - this.y);
                //egret.log("hit nextControl, this.mControlPointOffset:"+this.getPosStr(this.mControlPointOffset)+" thisPos:"+this.getPosStr(this)+" newPos:"+this.getPosStr(newPos));
                this.updateControlPoint();
                this.refreshConnectLines();
            }
        }
        ...
        //计算悬停在控制点上，0：代表没有悬停，-1:代表悬停在向前的控制点上， 1:代表悬停在向后的控制点上.
        public getHoverType(x, y):ControlHoverType{
            if(this.mIsSelected == false){
                if(this.isInDotRange(x, y, new egret.Point(this.x, this.y), ControlPointView.Radius+2)){
                    return ControlHoverType.ControlHoverType_Self;
                }
                return ControlHoverType.ControlHoverType_None;
            }
            //相同点，优先任务选中的是下一个控制点...
            if(this.isInDotRange(x, y, this.mControlNextPoint, ControlPointView.Radius+2)){
                return ControlHoverType.ControlHoverType_NextControlDot;
            }
            else if(this.isInDotRange(x, y, this.mControlLastPoint, ControlPointView.Radius+2)){
        ...
        protected isInDotRange(x, y, dstPos:egret.Point, range:number):boolean{
            if((x-dstPos.x)*(x-dstPos.x) + (y-dstPos.y)*(y-dstPos.y) <= range*range){
                return true;
            }
            return false;
        }
        ...
        public get ControlLastPoint():egret.Point{
            return this.mControlLastPoint;
        }

        public get ControlNextPoint():egret.Point{
            return this.mControlNextPoint;
        }

        //控制偏移点;
        public get ControlOffset():egret.Point{
            return this.mControlPointOffset;
        }
        ...
        public refreshLastLine(type?){
            ...
            if(this.lastCtrlPoint != null){
                this.mLastLine = this.newBezierLine(new egret.Point(this.lastCtrlPoint.x, this.lastCtrlPoint.y),
                    this.lastCtrlPoint.ControlNextPoint, this.ControlLastPoint,
        ...
        public refreshState(){
            ...
            this.mControlLastLine.visible = this.mIsSelected;
            this.mControlNextLine.visible = this.mIsSelected;
            this.mControlLastDot.visible = this.mIsSelected;
            this.mControlNextDot.visible = this.mIsSelected;
        }
        ...
        protected newBezierLine(fromPt, ctrl1Pt, ctrl2Pt, dstPt, type?, color:number=0xff0000, lineBold:number=2):egret.Shape{
            let line: egret.Shape = new egret.Shape();
            line.graphics.lineStyle(lineBold, type?0x0037FF:color);
            line.graphics.moveTo(fromPt.x, fromPt.y);
            line.graphics.cubicCurveTo(ctrl1Pt.x, ctrl1Pt.y, ctrl2Pt.x, ctrl2Pt.y, dstPt.x, dstPt.y);
            line.x = 0;
            line.y = 0;
            // line.alpha = 0.5;
            return line;      
        ...
        public getExportPaths():Array<{x,y}>{
            if(this.lastCtrlPoint == null){
                return [{x:this.x, y:this.y}];
            }
            egret.log("call ControlPoint, getExportPath");
            let lastPos = new egret.Point(this.lastCtrlPoint.x, this.lastCtrlPoint.y);
            let nextPos = new egret.Point(this.x, this.y);
            let nCount = Math.floor(lastPos.subtract(nextPos).length/ControlPointView.EXPORT_LINE_INTERVAL_DISTANCE);
            if(nCount > 10){
                nCount = 10;
            }
            let ctrlPos1 = this.lastCtrlPoint.ControlNextPoint;
            let ctrlPos2 = this.ControlLastPoint;

            let result:Array<{x,y}> = [];
            for(let i:number=1; i<=nCount; i++){
                let t:number = i/nCount;
                let px:number = (1-t)*(1-t)*(1-t)*lastPos.x + 3*(1-t)*(1-t)*t*ctrlPos1.x + 3*(1-t)*t*t*ctrlPos2.x + t*t*t * nextPos.x;
                let py:number = (1-t)*(1-t)*(1-t)*lastPos.y + 3*(1-t)*(1-t)*t*ctrlPos1.y + 3*(1-t)*t*t*ctrlPos2.y + t*t*t * nextPos.y;
    ...
    export class LineEditorView extends eui.Group{
        ...
        public static s_horizonOffset:number = 800;
        public static s_vertialOffset:number = 800;
        ...
        public closeEdit(){
            ...
            this.removeEventListener(egret.TouchEvent.TOUCH_MOVE, this.onTouchMove, this, true);
        ...
        protected onTouchBegin(event:egret.TouchEvent){
            ...
            console.error(JSON.stringify(newPos));
            if(this.mSelectCtrlPoint != null){
                if(this.mSelectCtrlPoint.getHoverType(newPos.x, newPos.y) != ControlHoverType.ControlHoverType_None){
            ...
            this.addNewControlPoint(newPos.x, newPos.y, 0, 0);
            this.mSelectCtrlPoint.onTouchBegin(event);
        }

        //添加一个新点;
        protected addNewControlPoint(x, y, cx, cy):ControlPointView{
            let newCtrl = new ControlPointView(x, y, false, cx, cy);
        ...
        //添加叠加点
         protected addNewControlPointNew(x, y, cx, cy, panel):ControlPointView{
            let newCtrl = new ControlPointView(x, y,true, cx, cy);
        ...
        public getHoverControlPoint(x, y):ControlPointView{
            ...
            while(listTemp != null){
                if(listTemp.getHoverType(x, y) == ControlHoverType.ControlHoverType_Self){
                    return listTemp;
                }
                listTemp = listTemp.getNextCtrl();
            }
        ...
        //获取路径的数据;
        public getPathDatas():Array<{x,y,cx,cy}>{
            let result:Array<{x, y, cx, cy}> =new Array<{x, y, cx, cy}>();
            let listTemp = this.mHeadCtrlView;
            while(listTemp != null){
                result.push({x:listTemp.x-this.anchorOffsetX, y:listTemp.y-this.anchorOffsetY, cx:listTemp.ControlOffset.x, cy:listTemp.ControlOffset.y});
                listTemp = listTemp.getNextCtrl();
            }
            return result;
        }
        ...
        //获取导出的路径数据;
        public getExportPathDatas():Array<{x, y}>{
           let result:Array<{x, y}> = new  Array<{x, y}>();
            let listTemp = this.mHeadCtrlView;
            while(listTemp != null){
                let paths = listTemp.getExportPaths();
        ...
        public setPathDatas(data:Array<{x, y, cx, cy}>){
            this.clear();
            for(let i:number=0; i<data.length; i++){
                this.addNewControlPoint(data[i].x+this.anchorOffsetX, data[i].y+this.anchorOffsetY, data[i].cx, data[i].cy);
            }
            this.operCmdList = [];
        }

        public setPathDatasNew(data:Array<{x, y, cx, cy}>,panel){
            this.mHeadCtrlView = null;
            this.mTailCtrlView = null;
            ...
            for(let i:number=0; i<data.length+1; i++){
                if(i == data.length){
                    ...
                    this.addNewControlPointNew(data[i].x+groupNew.anchorOffsetX, data[i].y+groupNew.anchorOffsetY, data[i].cx, data[i].cy, groupNew);
                }
            }
        }
